...
def check(scan_name, checks_subpath=None, config_file=None, data_source='snowflake', project_root='/usr/local/airflow/include/soda', context=None):
  ...
  with connect(user=user, password=password, account=account, warehouse=warehouse) as conn:
    ...
    for check in data.get('checks'):
        ...
        sql = f"INSERT INTO {db}.{schema}.data_quality_tests (NAME, DEFINITION, TABLE_NAME, OUTCOME, DIAGNOSTICS, SCAN_START, SCAN_NAME, FILE_PATH, FAIL_MESSAGES, LOAD_TIMESTAMP) SELECT '{name}', '{definition}', '{table_name}', '{outcome}', PARSE_JSON('{diagnostics}'), TO_TIMESTAMP_NTZ('{scan_start}'), '{scan_name}', '{file_path}', {fail_objects}, TO_TIMESTAMP_NTZ('{load_timestamp}');"
        try:
          cursor = conn.cursor()
          cursor.execute(sql)
          conn.commit()
